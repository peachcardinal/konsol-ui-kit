...
import type { ItemProps } from '../../combobox-primitive/types'

import { cn } from '@/lib/utils'

import { ItemView } from '../../combobox-primitive/components/item-view'
...
function ComboboxItemRendererComponent<T extends object>({
  ...
  const { items, selectedItem, getItemProps, isItemSelected } = useComboboxMultiSelectContext<T>()
          ...
          if (isComponentWithDisplayName(listChild.type, 'Item')) {
            try {
              ...
              if (itemData) {
                ...
                const isSelected = isItemSelected ? isItemSelected(item.value) : (selectedItemKey === itemKey)
